...
export const EventCard = ({ event }: EventCardProps) => {
    ...
    return (
        <Link href={`/events/${event.id}`} className="block h-full group">
            <Card className="h-full flex flex-col transition-all duration-300 hover:-translate-y-1 hover:shadow-md border-neutral-200">
                <div className="aspect-[16/10] bg-neutral-800 relative overflow-hidden">
                    ...
                    <div className="absolute top-4 right-4 z-20">
                        <div className="bg-white/90 backdrop-blur-sm border border-neutral-200 px-3 py-1.5 rounded-full flex items-center gap-2 shadow-sm">
                        ...
                        {event.vipPrice && (
                            <div className="bg-amber-100 border border-amber-200 px-3 py-1.5 rounded-full mt-2">
                    ...
                    <div className="mt-auto flex items-center justify-between pt-4 border-t border-neutral-100">
                        <div className="flex flex-col">
                            ...
                            <span className="font-bold text-neutral-900 text-lg">
                                {new Intl.NumberFormat('fr-FR').format(event.price)} <span className="text-xs text-neutral-500 font-normal">{event.currency}</span>
                            </span>
                        </div>
